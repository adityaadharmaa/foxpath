    ...
    <div className="min-h-screen bg-white dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-50 selection:bg-blue-100 dark:selection:bg-blue-900 transition-colors duration-300">
              ...
              {isLoggedIn ? (
                ...
                <>
                  <Button
                    ...
                    className="font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
                    Masuk
                  </Button>
                  <Button
                    onClick={() => navigate("/register")}
                    className="bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-full px-6 shadow-md hover:shadow-lg transition-all"
                  >
                    Daftar Akun
                  </Button>
                </>
      ...
      <section className="pt-40 pb-20 px-4 text-center max-w-5xl mx-auto relative overflow-hidden">
        ...
        <p className="max-w-2xl mx-auto text-lg text-slate-500 dark:text-slate-400 mb-10 leading-relaxed">
          Platform seleksi magang yang{" "}
          <strong>Transparan, Objektif, dan Adil</strong> menggunakan metode
          komputasi <em>Simple Additive Weighting (SAW)</em> untuk menemukan
          talenta terbaik.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <Button
            variant="ghost"
            className="h-14 px-8 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 w-full sm:w-auto"
          >
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">
              Mengapa FoxPath Berbeda?
            </h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
              Kami menggunakan teknologi SAW untuk memastikan setiap pelamar
              dinilai berdasarkan data, bukan asumsi.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Setiap kriteria (IPK, Skill, Portfolio) memiliki bobot matematis yang jelas.",
                color: "blue",
              },
              {
                icon: Users,
                title: "Transparansi Ranking",
                desc: "Hasil seleksi ditampilkan berdasarkan skor akhir tertinggi secara transparan.",
                color: "indigo",
              },
              {
                icon: Award,
                title: "Sertifikasi Resmi",
                desc: "Peserta yang lolos akan mendapatkan pengalaman kerja riil dan sertifikat.",
                color: "teal",
              },
            ].map((item, idx) => (
              <div
              ...
              >
                <div
                  className={`h-12 w-12 bg-${item.color}-100 dark:bg-${item.color}-900/30 rounded-2xl flex items-center justify-center text-${item.color}-600 dark:text-${item.color}-400 mb-6`}
                >
                  <item.icon size={24} />
                </div>
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Ikuti 4 langkah mudah untuk memulai karir impian Anda.
            </p>
          </div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-5xl mx-auto bg-slate-900 dark:bg-slate-900 border border-transparent dark:border-slate-800 rounded-[2.5rem] p-12 text-center relative overflow-hidden">
          ...
          <p className="text-slate-300 mb-8 max-w-xl mx-auto relative z-10">
            Jangan lewatkan kesempatan untuk berkembang bersama kami. Kuota
            terbatas untuk batch bulan ini.
          </p>
          <Button
            onClick={handleCtaClick}
      ...
      <footer className="py-10 border-t border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-950 text-center transition-colors duration-300">
        <div className="flex justify-center items-center gap-2 mb-4">
          <div className="h-6 w-6 rounded bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center text-xs font-bold">
            FP
          </div>
        ...
        <p className="text-slate-400 text-sm">
          &copy; {new Date().getFullYear()} Foxbyte Global Inovasi. All rights
          reserved.
        </p>
      </footer>
    </div>
