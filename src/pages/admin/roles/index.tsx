...
import { 
    Search, Edit, Trash2, Shield, Plus, Users, CheckCircle, ListFilter, Loader2,
    ChevronLeft, ChevronRight, Columns, Check, MoreHorizontal,
    Trash
} from "lucide-react";
...
function RoleActionMenu({onEdit, onDelete, isProtected}: {onEdit: () => void, onDelete: () => void, isProtected: boolean}) {
    const [isOpen, setIsOpen] = useState(false)
    const menuRef = useRef<HTMLDivElement>(null)
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Atur hak akses dan tipe pengguna dalam sistem.</p>
                </div>
                <Button onClick={openCreateModal} className="bg-blue-600 hover:bg-blue-700 text-white gap-2 shadow-lg shadow-blue-600/20">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div className="relative w-full sm:w-80 group">
                        ...
                        <Input 
                            ...
                            startIcon={<Search className="absolute top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within:text-blue-500 transition-colors" size={14}/>}
                            value={search}
                ...
                <div className="overflow-x-auto relative">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-950/50 uppercase text-[11px] tracking-wider font-semibold text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
                            <tr>
                                {visibleColumns.find(c => c.key === 'name')?.visible && <th className="px-6 py-4">Role Name</th>}
                                {visibleColumns.find(c => c.key === 'desc')?.visible && <th className="px-6 py-4">Description</th>}
                                {visibleColumns.find(c => c.key === 'count')?.visible && <th className="px-6 py-4">Users Count</th>}
                                {visibleColumns.find(c => c.key === 'date')?.visible && <th className="px-6 py-4">Created At</th>}
                                    ...
                                    <tr key={role.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        {visibleColumns.find(c => c.key === 'name')?.visible && (
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-semibold text-slate-900 dark:text-white uppercase">{role.name}</span>
                                                </div>
                                            </td>
                                        )}
                                        ...
                                        {visibleColumns.find(c => c.key === 'date')?.visible && (
                                            <td className="px-6 py-4 text-slate-500 text-xs">
                                                {role.created_at ? new Date(role.created_at).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'}) : '-'}
                                            </td>
                                        )}
                ...
                <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-50/50 dark:bg-slate-950/30">
                    <p className="text-xs text-slate-500">
                        Menampilkan <span className="font-medium text-slate-900 dark:text-white">{displayedRoles.length}</span> dari <span className="font-medium text-slate-900 dark:text-white">{pagination.total}</span> data
                    </p>
...
function StatCard({ title, value, icon, border, indicator, subtitle }: any) {
    return (
        <div className={`p-6 rounded-2xl bg-white dark:bg-slate-900 border shadow-sm ${border || 'border-slate-200'} relative overflow-hidden`}>
