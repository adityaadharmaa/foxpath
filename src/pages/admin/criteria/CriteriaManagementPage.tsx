...
import { Archive, ArrowDownCircle, ArrowUpCircle, Check, ChevronLeft, ChevronRight, Edit, Layers, Loader2, MoreHorizontal, Percent, Plus, RefreshCcw, Search, ToggleLeft, ToggleRight, Trash2 } from "lucide-react";
...
function ColumnToggle({columns, onChange} : {columns: any[], onChange: (key:string) => void}) {
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div 
                    className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-1"
                >
                    <p className="px-3 py-2 text-xs font-semibold text-slate-500 uppercase">Toggle Columns</p>
                    {columns.map((col) => (
                        <button key={col.key} onClick={() => onChange(col.key)} className="w-full text-left px-3 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center justify-between rounded-md">
                            {col.label}
...
function CriteriaActionMenu({criteria, onEdit, onDelete, onRestore, onToggle}: any) {
    const [isOpen, setIsOpen] = useState(false)
    const menuRef = useRef<HTMLDivElement>(null)
                        ...
                        {criteria.is_deleted ? (
                            ...
                            <>
                                ...
                                <button onClick={() => {onToggle(); setIsOpen(false)}} className="w-full text-left px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-2 rounded-lg">
                                    {criteria.is_active ? <ToggleRight size={14} className="text-green-500" /> : <ToggleLeft size={14} className="text-slate-400" />}
...
export default function CriteriaManagementPage() {
    ...
    const handleToggle = async (id: number) => {
        ...
        try{
            ...
            toast.error(error.respinse?.data?.message || "Gagal update status.", {id: toastId})
        }
    }
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Criteria Management</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Kelola kriteria penilaian (SAW), bobot, dan atribut.</p>
                </div>
                <div className="flex gap-2">
                   <Button 
                    ...
                    <Button onClick={() => { setCriteriaToEdit(null); setIsModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white gap-2 shadow-lg shadow-blue-600/20">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 uppercase text-[11px] tracking-wider font-semibold text-slate-500 border-b border-slate-200">
                            <tr>
                                {visibleColumns.find(c => c.key === 'info')?.visible && <th className="px-6 py-4">Criteria Info</th>}
                                {visibleColumns.find(c => c.key === 'weight')?.visible && <th className="px-6 py-4">Weight</th>}
                                {visibleColumns.find(c => c.key === 'type')?.visible && <th className="px-6 py-4">Type</th>}
                                {visibleColumns.find(c => c.key === 'status')?.visible && <th className="px-6 py-4">Status</th>}
                                {visibleColumns.find(c => c.key === 'action')?.visible && <th className="px-6 py-4">Action</th>}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {isLoading ? (
                                ...
                                <tr><td colSpan={5} className="px-6 py-20 text-center text-slate-500">Data tidak ditemukan.</td></tr>
                            ) : (
                                criterias.map((item) => (
                                   <tr key={item.id} className={`group transition-colors ${item.is_deleted ? 'bg-red-50/50' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                                    {visibleColumns.find(c => c.key === 'info')?.visible && (
                                        <td className="px-6 py-4 align-top">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className={`block font-semibold text-slate-900 dark:text-white ${item.is_deleted ? 'text-red-600 line-through' : ''}`}>
                                                        {item.name}
                                                    </span>
                                                    <span className="block text-xs text-slate-500 mt-0.5 truncate max-w-62.5" title={item.description}>
                                                        {item.description || '-'}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <td className="px-6 py-4 align-top">
                                            {item.is_deleted ? (
                                                <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-red-100 text-red-700">Deleted</span>
                                                 ...
                                                 <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-green-50 text-green-700 border border-green-200">Active</span>
                                            ) : (
                                                 <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-slate-100 text-slate-500 border border-slate-200">Inactive</span>
                                            )}
                                        </td>
                                    )} 
                                    ...
                                    {visibleColumns.find(c => c.key === 'action')?.visible && (
                                            <td className="px-6 py-4 text-right align-top">
                                                <CriteriaActionMenu 
                                                    criteria={item}
                                                    onEdit={() => { setCriteriaToEdit(item); setIsModalOpen(true); }}
                ...
                <div className="px-4 py-2 border-t border-slate-200 flex justify-between items-center bg-slate-50/50">
                    <p className="text-xs text-slate-500">Showing {criterias.length} of {pagination.total} data</p>
                    <div className="flex gap-2">
                        <Button
