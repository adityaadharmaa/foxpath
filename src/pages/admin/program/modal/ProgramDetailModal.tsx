...
import { Calendar, CalendarClock, User, X } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import PlacementEdtiDialog from "./PlacementEditDialog";
...
export default function ProgramDetailModal({
  ...
  const [selectedApplicantForPlacement, setSelectedApplicntForPlacement] =
    useState<any>(null);
        ...
        <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Informasi program dan daftar pelamar.
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-8">
              <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                  ...
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
                    <span className="text-slate-400 mb-1 text-[10px] uppercase font-bold flex items-center gap-1">
                    ...
                    <div className="font-semibold text-slate-800 dark:text-white text-sm flex flex-col">
                      ...
                      <span className="text-slate-400 text-[10px] leading-none my-0.5">
                        s/d
                      </span>

                      <span
                        className={`${new Date(program.registration_ends_at) < new Date() ? "text-red-500" : ""}`}
                      >
                  ...
                  <div className="p-3 bg-white dark:bg-slate-800 rounded-lg shadow-sm">
                    ...
                    <span
                      className={`font-bold ${program.is_active ? "text-green-600" : "text-slate-500"}`}
                    >
              ...
              <div>
                <h4 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <User size={18} className="text-blue-500" /> Daftar Pelamar (
                            ...
                            <td className="px-4 py-3 text-center align-top">
                              {app.status === "accepted" && (
                                <Button
                                  ...
                                  onClick={() =>
                                    setSelectedApplicntForPlacement(app)
                                  }
                                  title="Ubah Tanggal Magang"
                                >
                ...
                {selectedApplicantForPlacement && (
                  <PlacementEdtiDialog
                    isOpen={!!selectedApplicantForPlacement}
                    onClose={() => setSelectedApplicntForPlacement(null)}
                    application={selectedApplicantForPlacement}
                    onSuccess={() => {
                      fetchDetail();
                    }}
                  />
...
function BadgeStatus({ status }: { status: string }) {
  const styles: any = {
    ...
    calculated: "bg-sky-50 text-sky-700 border-sky-200",
    rejected: "bg-red-50 text-red-700 border-red-200",
  };

  return (
    <span
      className={`px-2 py-1 rounded-full text-xs font-bold uppercase border ${styles[status] || "bg-slate-100"}`}
    >
      {status}
    </span>
  );
