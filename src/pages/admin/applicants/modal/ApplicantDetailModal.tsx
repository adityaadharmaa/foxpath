...
export default function ApplicantDetailModal({ isOpen, onClose, applicationId }: ApplicantDetailModalProps) {
    ...
    const fetchDetail = async () => {
        setIsLoading(true);
        try {
            const response = await applicantService.getApplicationDetail(applicationId!);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] mx-4">
                ...
                <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-10">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900 dark:text-white">Detail Pelamar</h2>
                        <p className="text-xs text-slate-500">Informasi lengkap, hasil seleksi, dan dokumen.</p>
                    </div>
                            ...
                            <div className="lg:col-span-1 space-y-6">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="flex flex-col items-center text-center mb-4">
                                        <div className="h-24 w-24 rounded-full bg-slate-100 border-2 border-white shadow-sm overflow-hidden mb-3">
                                            {data.user?.profile?.profile_picture ? (
                                                <img src={getFileUrl(data.user.profile.profile_picture)} className="w-full h-full object-cover" alt="Profile" />
                                            ) : (
                                                <div className="w-full h-full flex items-center justify-center text-slate-400"><User size={40} /></div>
                                            )}
                                        </div>
                                        <h3 className="font-bold text-lg text-slate-900 dark:text-white">{data.user?.profile?.full_name || "Tanpa Nama"}</h3>
                                        ...
                                        <div className="mt-4">
                                            <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border ${
                                                ...
                                                data.status === 'verified' ? 'bg-blue-100 text-blue-700 border-blue-500' :
                                    ...
                                    <div className="space-y-3 text-sm border-t border-slate-100 dark:border-slate-800 pt-4 mt-4">
                                        ...
                                        <div className="flex items-start gap-3 text-slate-600 dark:text-slate-400">
                                            ...
                                            <span className="line-clamp-2">{data.user?.profile?.address || "-"}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                                    ...
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2 relative z-10">
                                        <Trophy size={18} className="text-yellow-500" /> Hasil Seleksi (SAW)
                                    </h4>
                                    
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg text-center">
                                            <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Rank</p>
                                            <div className="flex items-center justify-center gap-1">
                                                <span className="text-2xl font-bold text-slate-900 dark:text-white">#{data.rank || "-"}</span>
                                            </div>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg text-center">
                                            <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Final Score</p>
                                            <div className="flex items-center justify-center gap-1">
                                                <Star size={16} className="text-yellow-500 fill-yellow-500" />
                                                <span className="text-2xl font-bold text-slate-900 dark:text-white">
                                                    {data.final_score ? Number(data.final_score).toFixed(4) : "-"}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                        <div className="flex justify-between text-xs mb-1">
                                            ...
                                            <span className="font-medium text-slate-700 dark:text-slate-300">{formatDate(data.scored_at)}</span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                            <span className="text-slate-500">Decision Date:</span>
                                            <span className="font-medium text-slate-700 dark:text-slate-300">{formatDate(data.decided_at)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h4 className="font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                                        <GraduationCap size={18} className="text-blue-500" /> Pendidikan
                                    </h4>
                                    {data.user?.profile_education ? (
                                        <div className="space-y-3 text-sm">
                                            <div>
                                                <p className="text-xs text-slate-500 mb-0.5">Institusi</p>
                                                <p className="font-medium text-slate-800 dark:text-slate-200">{data.user.profile_education.institution_name}</p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    <p className="text-xs text-slate-500 mb-0.5">Jurusan</p>
                                                    <p className="font-medium text-slate-800 dark:text-slate-200">{data.user.profile_education.major}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-slate-500 mb-0.5">IPK / Nilai</p>
                                                    <span className="inline-block px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs font-bold">
                                                        {data.user.profile_education.gpa || data.user.profile_education.average_score || "-"}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ) : (
                                        <p className="text-sm text-slate-400 italic">Data pendidikan belum lengkap.</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                        <h4 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                            <Calendar size={18} className="text-indigo-500" /> Informasi Program
                                        </h4>
                                        <span className="text-xs text-slate-500">ID: #{data.id}</span>
                                    </div>
                                    <div className="p-5">
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="text-xl font-bold text-indigo-700 dark:text-indigo-400">{data.program?.name}</h3>
                                        ...
                                        <p className="text-sm text-slate-600 dark:text-slate-400 mb-5 leading-relaxed">{data.program?.description}</p>
                                        
                                        {/* Timeline Pelamaran */}
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                                            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700">
                                                <p className="text-xs text-slate-500 mb-1">Tanggal Melamar</p>
                                                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{formatDate(data.submitted_at)}</p>
                                            </div>
                                            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700">
                                                <p className="text-xs text-slate-500 mb-1">Verifikasi Admin</p>
                                                <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{formatDate(data.verified_at)}</p>
                                            </div>
                                            ...
                                            <div className={`p-3 rounded-lg border ${
                                                ...
                                                <p className={`text-xs mb-1 ${isRejected ? 'text-red-600' : 'text-slate-500'}`}>
                                                    {isRejected ? 'Ditolak Pada' : 'Diterima Pada'}
                                                </p>
                                                <p className={`text-sm font-semibold ${isRejected ? 'text-red-700' : 'text-slate-800 dark:text-slate-200'}`}>
                                                    {/* Jika Rejected ambil decided_at, Jika Accepted ambil admitted_at */}
                                                    {formatDate(isRejected ? data.decided_at : data.admitted_at)}
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        {isRejected && (
                                            <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4 flex gap-3">
                                                ...
                                                <div className="text-sm text-red-800 dark:text-red-300">
                                                    <p className="font-bold mb-1">Hasil Seleksi: Tidak Lolos</p>
                                                    <p>
                                                        Mohon maaf, peringkat akhir Anda adalah <strong>#{data.rank}</strong>. 
                                                        Program ini hanya menerima <strong>{data.program?.capacity}</strong> kandidat terbaik sesuai kuota yang tersedia.
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {isAccepted && (data.placement_start_at || data.placement_end_at) && (
                                            <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4 border border-indigo-100 dark:border-indigo-800">
                                                ...
                                                <div className="flex items-center gap-3 text-sm">
                                                    <div className="flex-1">
                                                        <span className="block text-xs text-indigo-600/70 dark:text-indigo-400/70 uppercase font-bold">Mulai</span>
                                                        <span className="font-medium text-slate-900 dark:text-white">{formatDateShort(data.placement_start_at)}</span>
                                                    </div>
                                                    <div className="text-indigo-300">➜</div>
                                                    <div className="flex-1">
                                                        <span className="block text-xs text-indigo-600/70 dark:text-indigo-400/70 uppercase font-bold">Selesai</span>
                                                        <span className="font-medium text-slate-900 dark:text-white">{formatDateShort(data.placement_end_at)}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                ...
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                                        <FileText size={20} className="text-orange-500" /> Dokumen Pendukung
                                    </h4>
                                    
                                    {data.documents && data.documents.length > 0 ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {data.documents.map((doc: any, idx: number) => (
                                            <div key={idx} className={`group p-4 bg-white dark:bg-slate-900 border rounded-xl transition-all shadow-sm ${
                                                doc.status === 'approved' ? 'border-green-200 bg-green-50/30' : // Ganti 'valid' -> 'approved'
                                                doc.status === 'rejected' ? 'border-red-200 bg-red-50/30' :     // Ganti 'invalid' -> 'rejected'
                                                    ...
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        ...
                                                        <div className="overflow-hidden">
                                                            <p className="font-semibold text-sm truncate text-slate-800 dark:text-slate-200 capitalize">
                                                                {doc.type ? doc.type.replace(/_/g, ' ') : 'Dokumen'}
                                                            </p>
                                                            
                                                            {/* Status Badge */}
                                                            <div className="flex items-center gap-1 mt-0.5">
                                                                {doc.status === 'approved' && <span className="text-[10px] font-bold text-green-600 flex items-center gap-1"><CheckCircle2 size={10}/> APPROVED</span>}
                                                                {doc.status === 'rejected' && <span className="text-[10px] font-bold text-red-600 flex items-center gap-1"><XCircle size={10}/> REJECTED</span>}
                                                                {(!doc.status || doc.status === 'pending') && <span className="text-[10px] font-bold text-orange-600 flex items-center gap-1"><AlertCircle size={10}/> PENDING</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <a href={getFileUrl(doc.file_path)} download
                                                    ...
                                                    {isApplicationEditable && doc.status === 'pending' ? (
                                                        ...
                                                        <div className="pt-2 mt-2 border-t border-slate-100 dark:border-slate-800 text-center">
                                                            {doc.status === 'pending' ? (
                                                                <span className="text-[10px] text-slate-400 italic">Menunggu (Aplikasi {data?.status})</span>
                                                            ) : (
                                                                <span className={`text-[10px] font-bold italic ${doc.status === 'approved' ? 'text-green-600' : 'text-red-600'}`}>
                                                                    {doc.status === 'approved' ? 'Dokumen Disetujui' : 'Dokumen Ditolak'}
                                                                </span>
                                                            )}
                                                        </div>
                                                    )}
                                        ...
                                        <div className="p-8 border-2 border-dashed border-slate-200 rounded-xl text-center bg-slate-50/50">
                                            <p className="text-slate-400 text-sm">Tidak ada dokumen yang dilampirkan.</p>
                                        </div>
                                    )}
                                </div>
