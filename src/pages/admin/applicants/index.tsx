...
export default function ApplicantManagementPage() {
  ...
  const openScoringModal = (applicant: any) => {
    setApplicantToScore(applicant);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "verified":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-600">
            <FileCheck size={14} /> Verified
          </span>
        );
      case "scored":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-violet-50 text-violet-700 border border-violet-600">
            <ClipboardCheck size={14} /> Scored
          </span>
        );
      case "calculated":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-sky-50 text-sky-700 border border-sky-600">
            <Cpu size={14} /> Calculated
          </span>
        );
      default:
        return (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Pantau semua lamaran masuk dari berbagai program.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto flex-1">
            <div className="relative w-full sm:w-80 group">
              <Input
                ...
                startIcon={
                  <Search className="absolute top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
                }
                value={search}
                onChange={(e) => {
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isLoading ? (
                ...
                applicants.map((app) => (
                  ...
                  >
                    {/* Applicant Info */}
                    {visibleColumns.find((c) => c.key === "applicant")
                      ?.visible && (
                    ...
                    {/* Program Info */}
                    {visibleColumns.find((c) => c.key === "program")
                      ?.visible && (
                      <td className="px-6 py-4 align-top">
